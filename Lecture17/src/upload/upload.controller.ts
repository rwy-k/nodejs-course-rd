...
export class UploadController {
  ...
  async uploadAvatar(
    ...
    const uploaded = await this.uploadService.uploadAvatar(
      file,
      user.id,
      user.id,
    );
  ...
  async uploadProductImage(
    ...
    const uploaded = await this.uploadService.uploadProductImage(
      file,
      user.id,
      productId,
    );
  ...
  async getProductFiles(@Param('productId', ParseIntPipe) productId: number) {
    return this.uploadService.getFilesByEntity(
  ...
  async createPresignedUpload(
    ...
    if (
      dto.entityType === FileEntityType.PRODUCT &&
  ...
  async completeUpload(
    ...
    const updatedRecord = await this.uploadService.confirmUpload(
      fileId,
      dto.size,
    );
