...
Глобально за замовчуванням: **200 запитів / 60 с** на IP для всього, що не `@SkipThrottle()` і не перебите жорсткішим декоратором. Налаштування збираються в `ThrottlerModule`, guard — `src/common/guards/app-throttler.guard.ts` (там же підтримка GraphQL, якщо раптом знадобиться).

Окремі buckets (константи в `src/config/throttle.config.ts`):

| Що | Ліміт | Де |
| --- | --- | --- |
| `THROTTLE_AUTH_STRICT` | 10 / 60 с | `POST /auth/login`, `POST /auth/register`, `POST /auth/refresh` |
...
| `THROTTLE_GRAPHQL_MUTATION` | 25 / 60 с | наприклад `clientPing` в `HelloResolver` — запас «на виріст» для чутливих мутацій; queries живуть на глобальному ліміті |

## Helmet

Підключено в `src/main.ts` через `helmet()`.

У **dev** CSP вимикаємо (`contentSecurityPolicy: false`), бо інакше страждають Swagger і GraphQL Playground (inline scripts). У **production** CSP лишаємо дефолтний від Helmet — Playground у prod у нас і так вимкнений в `app.module.ts`.

`crossOriginEmbedderPolicy: false` — менше конфліктів з дев-тулами.
