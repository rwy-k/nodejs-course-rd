...
interface PaymentsClient {
  ...
  getPaymentStatus(data: GetPaymentStatusInput): import('rxjs').Observable<GetPaymentStatusResult>;
}
...
export class PaymentClientService implements OnModuleDestroy {
  ...
  constructor(
    ...
    this.retryAttempts = this.config.get<number>('paymentsClient.retryAttempts');
    this.retryInitialMs = this.config.get<number>('paymentsClient.retryInitialMs');
  ...
  private async executeWithRetry<T>(
    ...
    for (let attempt = 0; attempt <= this.retryAttempts; attempt++) {
      try {
        ...
        lastError = err;
        const code = err instanceof TimeoutError
    ...
    const code = lastError instanceof TimeoutError
      ? GrpcStatusCode.DEADLINE_EXCEEDED
