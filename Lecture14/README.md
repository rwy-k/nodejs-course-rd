# Lecture 14 — Orders + Payments (gRPC)

Два NestJS-сервіси: **Orders** (HTTP) викликає **Payments** (gRPC) через контракт `.proto`.
...
## Критерії оцінювання (чекліст)

**Correctness:** payments-service запускається окремо; є payments.proto з service + messages; Authorize і GetPaymentStatus за контрактом; orders викликає Payments через gRPC client без імпорту коду Payments; є happy path Orders → Authorize → paymentId/status.

**Resilience:** на gRPC-виклику з Orders встановлено deadline/timeout; timeout береться з env/config (`PAYMENTS_GRPC_TIMEOUT_MS` → `paymentsClient.grpcTimeoutMs`).

**Code quality:** конфіги/порти/адреси в env (Orders: `src/config/payments-client.config.ts`; Payments: `apps/payments-service/src/config.ts`); структура зрозуміла — див. таблицю «Структура» вище.
