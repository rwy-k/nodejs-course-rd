# Як перевірити, що hardening реально працює

Цей файл — для того, щоб швидко відтворити перевірки і зібрати артефакти. Частина того ж лежить у `**security-homework/security-evidence/**` (headers, rate limit, приклад audit, нотатки по секретам і TLS). Детальніше по темах: `RATE-LIMIT-AND-HEADERS.md`, `SECRETS.md`, `TRANSPORT-TLS.md`, `AUDIT.md`.

**Перед тестами**  треба підняти API: наприклад `docker compose -f docker-compose.local.yml up -d` і сервіс `orders-api`, або `npm run start:dev` з нормальним `.env`. Нижче порти як у локальному compose для API: `**http://localhost:8080`**.

---

## 1. Security headers

У `main.ts` підключений Helmet. У dev CSP вимкнений (як у типовому Express+Helmet прикладі), щоб не ламати дев-тули; у production CSP знову вмикається.

Приклад того, що побачила на Express 4 + `helmet@8` (той самий пакет, що в Nest):
...
Якщо розглядати nginx — переконайся, що летить `X-Forwarded-For` (див. `docker/nginx/local-gateway.conf`), інакше ліміт може рахуватись не по клієнту. У коді для цього є `trust proxy` + `clientIpTracker`, розписано в `RATE-LIMIT-AND-HEADERS.md`.

---

## 3. Audit (хоча б кілька «важливих» подій)
...
У консолі шукай префікс логера `Audit` або `"type":"audit"`. Що саме не логуємо — в `AUDIT.md`.
