...
**Security homework (структура ДЗ):** [security-homework/SECURITY-BASELINE.md](security-homework/SECURITY-BASELINE.md) та [security-homework/security-evidence/](security-homework/security-evidence/). Розширені curl-приклади: [docs/HARDENING-EVIDENCE.md](docs/HARDENING-EVIDENCE.md).

### Hardening — коротка нотатка

- **Найслабше місце до hardening:** публічні чутливі дії (логін / реєстрація, `request-payment` без перевірки власника, admin payments) були обмежені переважно бізнес-логікою, без послідовного throttling за типом трафіку, структурованого audit, чіткої політики секретів / TLS на edge та базових security headers — тобто типовий «CRUD-сервіс без defence in depth».
- **Що виправили:** глобальний + точкові throttles (auth / payments / admin / GraphQL), Helmet + trust proxy для IP за gateway, audit на критичні події без чутливих payload’ів, bootstrap секретів (`*_FILE`, обов’язковий `JWT_SECRET`, пароль сідів лише з env), опис TLS на edge (Ingress / nginx), документацію та evidence (baseline, curl, приклади логів).
- **Що свідомо в backlog:** IDOR на `request-payment`, TLS/mTLS для gRPC між сервісами, immutable audit sink / SIEM, captcha / anomaly detection на логін, fine-grained scopes замість лише ролі ADMIN, CORS whitelist, GraphQL depth/cost, Vault / зовнішній secret manager як цільова зрілість.
...
У **`.env`** обов’язково задайте **`JWT_SECRET`** (підпис JWT) та **`SEED_ADMIN_PASSWORD`** (пароль для `admin@example.com` після сіду) — шаблон у **`.env.example`**, деталі в **`docs/SECRETS.md`**. Реальні значення не комітити.
...
1. Логін: `POST http://localhost:3000/auth/login` з тілом `{"email":"admin@example.com","password":"<ваш SEED_ADMIN_PASSWORD з .env>"}` → зберегти `accessToken`.
...
У репозиторії налаштований повний CI/CD pipeline для Lecture19 через GitHub Actions.
...
- Jobs:
  ...
  - **Deploy to Stage** (тільки для `develop`) → запускає `Lecture19/scripts/deploy-stage.sh`, який:
...
1. Розробка → створити гілку від `develop`, зробити зміни в Lecture19, відкрити PR у `develop`.
