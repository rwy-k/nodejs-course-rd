...
export class AddFileIdRelations1707800000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    const hasAvatarFileId = await queryRunner.hasColumn(
      'users',
      'avatarFileId',
    );
    if (!hasAvatarFileId) {
    ...
    const hasImageFileId = await queryRunner.hasColumn(
      'products',
      'imageFileId',
    );
    if (!hasImageFileId) {
    ...
    const hasUsersFk = usersTable?.foreignKeys.some((fk) =>
      fk.columnNames.includes('avatarFileId'),
    );
    if (!hasUsersFk) {
    ...
    const hasProductsFk = productsTable?.foreignKeys.some((fk) =>
      fk.columnNames.includes('imageFileId'),
    );
    if (!hasProductsFk) {
      await queryRunner.createForeignKey(
        'products',
